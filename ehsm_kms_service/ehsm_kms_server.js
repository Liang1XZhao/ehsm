...
const create_app_info = (DB, res) => {
  ...
  DB.insert({ appid: appid, appkey: appid }, appid)
    .then((r) => {
      res.send(_result(200, 'succ', { appid: appid, appkey: appid }))
    })
    .catch((e) => {
      res.send(_result(400, 'creat app info faild', e))
    })
}
...
const server = (DB) => {
  ...
  app.post('/ehsm', function (req, res) {
    try {
      ...
      } else if (ACTION === apis.RA_GET_API_KEY) {
        /**
         * RA_GET_API_KEY
         */
        create_app_info(DB, res)
      } else if (ACTION == apis.RA_HANDSHAKE_MSG0) {
        /**
         * RA_HANDSHAKE_MSG0
         */
